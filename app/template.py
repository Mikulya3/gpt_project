...
def generate_gpt(data,name,basic_info,major_preferences,college_preferences,potential_major_exploration,column_AU):
    ...
    # )
    # word_cloud=chat_completion.choices[0].message.content
    # print("done0")
    ...
    major_prompt_four =f"""请列出上个回答中推荐的三个专业的中文名称: use this information'''{generated_major_prompt_three}'''  """
    ...
    Correspondence_college_recommendations = f"""学生希望未来到'''{column_AU}'''国家读大学本科，可能专业为'''{major_list}'''；请你为他推荐这些国家中三个专业分别具有代表性的两所大学，并为每个大学分别给出详细的推荐理由，每个推荐理由不少于300字。每个推荐大学和下一个推荐大学中请加入空行。请在国家与国家之间加入空行"""
...
    综合考虑，根据Eva在不同学科的兴趣和五大维度的评估，推荐专业包括运动医学、康复科学和生物科学。在这些领域，Eva表现出较高的兴趣和匹配程度。她在生物学方面有较好的知识掌握程度和热爱程度，对于实践应用能力和未来投入意愿也表现出较高的需求和意愿。对于创新能力，Eva在这些学科中可能需要进一步培养和锻炼。建议Eva在选择专业时综合考虑自己的兴趣和综合能力，对未来职业规划有明确的认识，以做出最适合自己的决策。）"""
    ...
    print("done13")
    # return generated_Visualization_p1
...
def main():
    ...
    template_path = "/home/hello/VScode/template.tex"
    ...
    final_content = template_content.replace("%generated_summary_%", generated_summary)
    final_content = final_content.replace("%name_%", name)
